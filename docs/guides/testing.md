...
Note that the identity expectation uses `.same?` which tests if [`#object_id`][Reference#object_id] are identical. This is only true if the expected value points to *the same object* instead of *an equivalent one*. This is only possible for reference types and won't work for value types like structs or numbers.
