...
Note that `:foo` was the result of the interpolation, because that's what was passed to the macro. You can use the method [`ASTNode#id`][Crystal::Macros::ASTNode#id] in these cases, where you just need an identifier.
...
Similar to regular code, [`Nop`][Crystal::Macros::Nop], [`NilLiteral`][Crystal::Macros::NilLiteral] and a false [`BoolLiteral`][Crystal::Macros::BoolLiteral] are considered *falsey*, while everything else is considered *truthy*.
...
To iterate an [`ArrayLiteral`][Crystal::Macros::ArrayLiteral]:
...
To iterate a [`HashLiteral`][Crystal::Macros::HashLiteral]:
...
The arguments are packed into an [`ArrayLiteral`][Crystal::Macros::ArrayLiteral] and passed to the macro.

Additionally, using `*` when interpolating an [`ArrayLiteral`][Crystal::Macros::ArrayLiteral] interpolates the elements separated by commas:
...
When a macro is invoked you can access the current scope, or type, with a special instance variable: `@type`. The type of this variable is [`TypeNode`][Crystal::Macros::TypeNode], which gives you access to type information at compile time.
...
When a macro is invoked you can access the method, the macro is in with a special instance variable: `@def`. The type of this variable is [`Def`][Crystal::Macros::Def] unless the macro is outside of a method, in this case it's [`NilLiteral`][Crystal::Macros::NilLiteral].
...
If the constant denotes a type, you get back a [`TypeNode`][Crystal::Macros::TypeNode].
