...
For comparing an expression against a `case`'s value the *case equality operator* `===` is used. It is defined as a method on [`Object`][Object#===(other)] and can be overridden by subclasses to provide meaningful semantics in case statements. For example, [`Class`][Class#===(other)] defines case equality as when an object is an instance of that class, [`Regex`][Regex#===(other)] as when the value matches the regular expression and [`Range`][Range#===(value)] as when the value is included in that range.
